...
export default function ExportCitationEntry() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/citation-reports/export-entry', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        a.href = url;
        a.download = `citation-entries-${new Date().toISOString().split('T')[0]}.xlsx`;
  ...
  return (
    <div className={`w-full ${isRTL ? 'font-arabic' : ''}`} dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <div className="bg-gradient-to-r from-red-600 to-red-700 text-white p-6 rounded-t-xl">
        <h2 className="text-2xl font-bold mb-2">{t.reportContent.titles.citationEntry}</h2>
        <p className="text-red-100">{t.reportContent.descriptions.citationEntry}</p>
      </div>
