...
// رقم المجلة	اسم المجلة	الجهة	الحالة  
  const query = `
    SELECT 
      a.accountid AS 'ID',
      a.employees AS 'رقم المجلة',
      a.accountname AS 'اسم المجلة',
      cf.cf_883 AS 'العنوان بالانجليزية',
      cf.cf_885 AS 'العنوان المترجم',
      cf.cf_877 AS 'الدوله',
      cf.cf_703  AS 'الجهة', 
      cf.cf_707 AS 'الحالة',
      cf.cf_709 AS 'ISSN',
      cf.cf_715 AS 'حالة المجلة',
      cf.cf_717 AS 'عنوان سابق',
      cf.cf_719 AS 'عدد مرات الصدور',
      cf.cf_723 AS 'محكمة',
      cf.cf_725 AS 'النص الكامل',
      cf.cf_727 AS 'قاعدة البيانات',
      cf.cf_729 AS 'التخصص فرعي',
      cf.cf_873 AS 'رقم المجلة(الارتباط)',
      cf.cf_887 AS 'الرومنة',
      cf.cf_721 AS 'أعداد المجلة',
      cf.cf_875 AS 'الإتاحة',
      cf.cf_711 AS 'ISBN',
      cf.cf_901 AS 'Category - A',
      cf.cf_903 AS 'Category - E',
      cf.cf_905 AS 'Category - A2',
      cf.cf_907 AS 'Category - E2',
      cf.cf_931 AS 'Journal URL',
      cf.cf_919 AS 'LC-DAR',
      cf.cf_921 AS 'DOI Prefix',
      cf.cf_923 AS 'Expire Date',
      cf.cf_925 AS 'DOI Password',
      cf.cf_933 AS 'Category (Other Language)'
...
export async function getAllConferencesData(): Promise<any[]> {
  ...
  //Take the same fields names and order as magazines for consistency
  
  const query = `
    SELECT 
      a.accountid AS 'ID',
      a.employees AS 'رقم المؤتمر',
      a.accountname AS 'اسم المؤتمر',
      cf.cf_883 AS 'العنوان بالانجليزية',
      cf.cf_885 AS 'العنوان المترجم',
      cf.cf_877 AS 'الدوله',
      cf.cf_703  AS 'الجهة', 
